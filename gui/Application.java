package gui;
//AFL Editor v1.5 by ViveTheJoestar
...
public class Application {
	public static JProgressBar nameProgBar;
	...
	private static final String WINDOW_TITLE = "AFL Editor v1.5";
	...
	private static void setDisplayNames(JButton displayBtn) throws IOException {
		...
		if (pathList.length == 0)
			JOptionPane.showMessageDialog(null, HTML_TEXT_BTN + "The provided AFL directory contains no AFL files!",
					WINDOW_TITLE, 0);
		else {
			...
			//go through AFLs and only get the file names of correct ones
			for (int i = 0; i < pathList.length; i++) {
				Main.afl = afls[i];
				if (Main.isValidAFL()) {
					...
					invalidAflIndices[i] = i;
			}
			//shift a bunch of AFL-related info to effectivly skip faulty AFLS
			for (int i = 0; i < pathList.length; i++) {
				if (invalidAflIndices[i] != -1) {
					...
					System.arraycopy(aflNames, invalidAflIndices[i] + 1, newAflNames, invalidAflIndices[i],
							aflNames.length - invalidAflIndices[i] - 1);
					//shift AFL references
					System.arraycopy(afls, 0, newAfls, 0, invalidAflIndices[i]);
					System.arraycopy(afls, invalidAflIndices[i] + 1, newAfls, invalidAflIndices[i],
							aflNames.length - invalidAflIndices[i] - 1);
					//shift AFL name counts
					System.arraycopy(aflNameCnts, 0, newAflNameCnts, 0, invalidAflIndices[i]);
					System.arraycopy(aflNameCnts, invalidAflIndices[i] + 1, newAflNameCnts, invalidAflIndices[i],
							aflNames.length - invalidAflIndices[i] - 1);
					//apply shifted AFL names to the double array
					while (indexOfInvalidAfl + 1 < aflNames.length) {
						System.arraycopy(allFileNames[indexOfInvalidAfl + 1], 0, allFileNames[indexOfInvalidAfl], 0,
								65535);
						indexOfInvalidAfl++;
					}
			...
			applyBtn.setToolTipText(HTML_TEXT_TIP
					+ "If any file name contains more than 32 characters,<br>only its first 32 characters will be applied to the AFL.");
			table.setFont(PLAIN);
				...
				public void actionPerformed(ActionEvent e) {
					try {
						...
						JOptionPane.showMessageDialog(null,
								HTML_TEXT_BTN + "Any changes to the file names have been saved!", WINDOW_TITLE, 1);
					} catch (Exception ex) {
						errorBeep();
			...
			SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
				...
				protected Void doInBackground() {
					try {
						...
						for (int i = 0; i < afls.length; i++) {
							...
							if (Main.isValidAFL()) {
								...
								if (isApplyBtnClicked)
									...
									Main.fixDuplicateFileNames();
							} else
								fileLabel.setText(HTML_TEXT_2 + "Skipping:<br>" + aflName + "<br>");
						...
						DEF_TOOLKIT.beep();
						JOptionPane.showMessageDialog(null,
								HTML_TEXT_BTN + "AFL files have been overwritten successfully in " + time + " seconds!",
								WINDOW_TITLE, 1);
					} catch (Exception e) {
						errorBeep();
	...
	public static void setApplication() {
		...
		//set tooltips
		displayBtn.setToolTipText(HTML_TEXT_TIP
				+ "Opens up a dialog displaying all the file names from the AFL (which can also be edited).");
		replaceAllBtn
				.setToolTipText(HTML_TEXT_TIP + "Replaces all instances of the string for each file name in the AFL.");
		replaceFirstBtn.setToolTipText(
				HTML_TEXT_TIP + "Replaces only the first instance of the string for each file name in the AFL.");
		noDupliBtn.setToolTipText(HTML_TEXT_TIP
				+ "Adds the file ID at the end of each duplicate file name,<br>since AFS Explorer overwrites files with the same name.");
		...
		displayBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					if (src == null) {
						errorBeep();
						JOptionPane.showMessageDialog(null, HTML_TEXT_BTN + "No AFL directory has been selected!",
								WINDOW_TITLE, 0);
					} else {
